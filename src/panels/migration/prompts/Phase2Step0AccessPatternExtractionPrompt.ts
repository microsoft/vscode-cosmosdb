...
RULES:
- Extract EVERY access pattern described in the report unless instructed otherwise by the user in additional assessment instructions.
- Preserve the original pattern names, table names, and code references exactly as written.
  - For each pattern, determine the query type (read, write, or read-write) and TPS from the report content.
- If a field is not mentioned or not applicable, use "N/A".
...
- For read patterns, extract the Filter/Lookup Fields from the report into "filterFields".
- For write patterns, extract the Single/Batch value from the report into "singleOrBatch".
  ...
  "accessPatterns": [
    {
      ...
      "frequency": "string — the TPS as written in the report, or 'N/A' if not mentioned",
      "codeReferences": ["string — file names or paths referenced in the report"],
      "filterFields": "string | null — the Filter/Lookup Fields as written; null for pure write patterns",
      "singleOrBatch": "string | null — 'Single', 'Batch', or as written in the report; null for pure read patterns",
...
Do not include any explanation before or after the JSON.`,
            ...
            vscpp(
                ...
                vscpp(
                    TextChunk,
                    { priority: 88, breakOn: /\s+/g },
                    this.props.discoveryInstructions
                        ? '\n\n# Discovery Instructions\n\nThe following instructions were used to guide the discovery phase that produced the report above. Use them as additional context when extracting access patterns.\n\n' +
                              this.props.discoveryInstructions
                        : '',
                ),
                vscpp(
                    ...
                    this.props.assessmentInstructions
                        ? '\n\n# Additional Assessment Instructions\n\nApply these user instructions while extracting access patterns. If they explicitly exclude specific tables or schemas, follow those exclusions.\n\n' +
                              this.props.assessmentInstructions
                        : '',
                ),
            ),
