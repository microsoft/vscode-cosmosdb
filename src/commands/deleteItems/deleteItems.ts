...
export async function deleteDocumentDBItem(context: IActionContext, node: DocumentDBItemResourceItem): Promise<void> {
    ...
    if (item.id === undefined) {
        vscode.window.showErrorMessage(vscode.l10n.t('Item id is required'));
        return undefined;
    }

    const confirmed = await getConfirmationAsInSettings(
        vscode.l10n.t(`Delete item?`),
        vscode.l10n.t(`Delete item and its contents?\nThis can't be undone.`),
        item.id,
    );
    ...
    try {
        ...
        if (success) {
            showConfirmationAsInSettings(
                vscode.l10n.t('The item {0} has been deleted.', item.id ? `"${item.id}"` : ''),
            );
        }
    } finally {
