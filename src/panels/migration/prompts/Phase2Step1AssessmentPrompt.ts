...
You MUST include ALL domains in your response. Do NOT skip or omit any domain, UNLESS the
user's discovery or assessment instructions explicitly exclude specific tables or schemas —
in that case, omit those tables entirely and omit any domain whose tables are ALL excluded.

GROUNDING RULES:
- If the user's discovery instructions or assessment instructions explicitly exclude certain
  tables or schemas, remove those tables before determining domains. If every table that
  would belong to a domain has been excluded, do NOT create that domain at all — omit it
...
- Every non-excluded table from the dependency graph must appear in EXACTLY ONE domain.
  No table may be omitted or duplicated (unless explicitly excluded by user instructions).
  The total table count across all domains must equal the total non-excluded table count
...
7. **Apply Weighing Heuristics** — Use the following signal weights when deciding how to
   group tables into domains:

   | Signal | Weight | Rationale |
   |---|---|---|
   | Controller access patterns (read paths) | High | Drives collection/partition design directly |
   | Controller write patterns + transaction scope | High | Defines aggregate boundaries |
   | FK relationships traversed in code | Medium | Denormalize vs reference decision |
   | FK relationships not traversed in code | Low | Safe to break |
   | Table structure / column types | Medium | Informs document shape |
   | Tables unreferenced by any controller | Low | Decompose or deprecate separately |

8. **Trace Access Chains** — Trace controller → repository/DAO → query → table as a chain,
   not just inspect tables in isolation. This reveals the true data access topology.

9. **Flag Cross-Aggregate Queries** — Flag any query that joins 3+ tables in a single
   controller method as a "cross-aggregate" query. These are decomposition risk zones that
   indicate tables which may need to stay together or require careful denormalization.

10. **Identify Write Amplification Patterns** — Controllers that write to many tables
    atomically point at aggregate boundaries that must stay together. These patterns
    should strongly influence domain grouping.

11. **Score Tables by Access Cohesion** — Tables always accessed together score high for
    embedding within the same collection. Tables accessed independently score high for
...
IMPORTANT: Your FINAL response must be ONLY the JSON object. Do not wrap it in a code block.`,
            ...
            vscpp(
                ...
                vscpp(
                    TextChunk,
                    { priority: 90, breakOn: /\s+/g },
                    this.props.dependencyGraph || '(no dependency graph available)',
                ),
                vscpp(
                    TextChunk,
                    { priority: 87 },
                    this.props.assessmentInstructions
                        ? '\n\n# ADDITIONAL ASSESSMENT INSTRUCTIONS (from the user)\n\n' +
                              this.props.assessmentInstructions +
                              '\n\n'
                        : '',
                ),
                vscpp(
                    ...
                    this.props.discoveryInstructions
                        ? '\n\n# ADDITIONAL DISCOVERY INSTRUCTIONS (from the user)\n\nThe following instructions were used to guide the discovery phase that produced the report below:\n\n' +
                              this.props.discoveryInstructions +
                              '\n\n'
                        : '',
                ),
