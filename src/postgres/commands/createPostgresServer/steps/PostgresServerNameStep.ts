...
import { PostgresServerType, type AbstractNameAvailability } from '../../../abstract/models';
        ...
        context.newServerName = (
            await context.ui.showInputBox({
                ...
                validateInput: (name: string) =>
                    validatePostgresServerName(name, client, nonNullProp(context, 'serverType')),
            })
        ).trim();
...
async function validatePostgresServerName(
    name: string,
    ...
    const availability: AbstractNameAvailability = await client.checkNameAvailability.execute({
        name: name,
        type: resourceType,
    });
