...
import { FabricMirroredItemResourceItem } from './FabricMirroredItemResourceItem';

export class FabricMirroredItemsResourceItem extends FabricItemsResourceItem {
    ...
    protected getChildrenImpl(items: ItemDefinition[]): Promise<TreeElement[]> {
        return Promise.resolve(
            items.map(
                (item) => new FabricMirroredItemResourceItem(this.context, { ...this.model, item }, this.experience),
            ),
        );
    }
}
