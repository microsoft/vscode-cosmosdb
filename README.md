...
- Choose **Deploy now** (the default) or **Deploy with Biceps** in the final section.
  ...
  - **Deploy with Biceps** reveals the compact Monaco editor and copy/regenerate buttons for manual deployment with your own tools.
    The template is generated by the same helper as the Migration Assistant, with single and hierarchical partition keys.
...
In **Deploy with Biceps** mode you can edit and copy the template for manual deployment; the wizard does not deploy that template.
Switching to **Deploy now** uses only the selected model, never hidden Bicep edits. Export edits are kept if you switch back.
Changing the database or containers regenerates an unedited template only while Bicep mode is active; replacing custom
edits requires confirmation. Template-generation failures do not block direct deployment.
