...
const AUTO_REFRESH_INTERVALS_SECONDS = {
...
export type AccountOverviewState = ReturnType<typeof useAccountOverview>;
...
/** Shared data and action lifecycle; mount once above the active Overview presentation. */
export const useAccountOverview = () => {
    ...
    const inventoryMetadataRequestSeq = useRef(0);
    const partitionRequestSeq = useRef(0);
    ...
    const fetchPartitionHealth = useCallback(
        (container: ContainerRef, mode: PartitionDistributionMode, range: TimeRange) => {
            const requestId = ++partitionRequestSeq.current;
            ...
            return trpcClient.accountOverview.getPartitionHealth
                ...
                .then((result: PartitionHealthResult) => {
                    if (requestId === partitionRequestSeq.current) {
    ...
    const reloadInventory = useCallback(() => {
        const requestId = ++inventoryMetadataRequestSeq.current;
        return trpcClient.accountOverview.getInventory
            .query()
            .then((result: Inventory) => {
                if (requestId === inventoryMetadataRequestSeq.current) {
                    setInventory(result);
                }
            })
            .catch(() => {
    ...
    // Static metadata is loaded once and explicitly reloaded after resource creation.
    useEffect(() => {
        void fetchSummary();
        void reloadInventory();
        return () => {
            inventoryMetadataRequestSeq.current += 1;
        };
    }, [fetchSummary, reloadInventory]);
    ...
    const runAccountAction = useCallback(
        async (input: inferRouterInputs<AccountOverviewAppRouter>['accountOverview']['runAccountAction']) => {
            if (accountActionInFlight.current) {
                return;
            }
            ...
            try {
                await trpcClient.accountOverview.runAccountAction.mutate(input);
                if (input.action === 'createDatabase' || input.action === 'createContainer') {
        ...
        [trpcClient, reloadInventory, refresh],
    );
    ...
    return {
        ...
        accountActionFailed,
        refresh,
        autoRefreshIntervalsSeconds: AUTO_REFRESH_INTERVALS_SECONDS,
        actions,
        handleOpenUrl,
        handleRevealInTree,
