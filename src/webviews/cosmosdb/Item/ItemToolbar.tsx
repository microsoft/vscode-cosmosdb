...
import { useItemState } from './state/ItemContext';
...
export const ItemToolbar = (props: ItemToolbarProps) => {
    const state = useItemState();

    const isReady = state.isReady;
    const inProgress = state.isSaving || state.isRefreshing;
    const hasItemInDB = state.itemId !== '';
    const isReadOnly = isReady && state.mode === 'view'; // If the item is not initialized, it is considered as not state
            ...
            <Toolbar size={'small'}>
                ...
                >
                    <ToolbarButton
                        ...
                        disabled={inProgress || !hasItemInDB}
                    >
                        {l10n.t('Refresh')}
                    </ToolbarButton>
                </Tooltip>
