...
export class DataPlaneCosmosDBControlPlane implements CosmosDBControlPlane {
    public constructor(private readonly accountInfo: AccountInfo) {}

    public async listDatabases(): Promise<DatabaseResource[]> {
        return withClaimsChallengeHandling(this.accountInfo, async (client) => {
    ...
    public async deleteDatabase(databaseId: string): Promise<void> {
        await withClaimsChallengeHandling(this.accountInfo, async (client) => {
