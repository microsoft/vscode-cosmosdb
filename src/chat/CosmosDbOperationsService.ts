...
import {
    commentOutQuery,
    ensureQueryIsExecutableOrCommented,
    sanitizeSqlComment,
    stripCodeFences,
    ...
    public async generateQueryWithLLM(
        ...
        if (withExplanation) {
            ...
            const schemaSamplingComment = schemaSamplingExecuted
                ...
                : '';
            return {
                query: schemaSamplingComment + ensureQueryIsExecutableOrCommented(query),
        ...
        const schemaSamplingComment = schemaSamplingExecuted
            ? `-- ${l10n.t('Schema sampling tool was executed. Cost: {0} RUs', schemaSamplingRUs.toFixed(2))}\n`
            : '';
        return schemaSamplingComment + ensureQueryIsExecutableOrCommented(this.cleanupQueryResponse(responseText));
    }
