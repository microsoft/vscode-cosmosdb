...
| STR-12 | `SELECT TRIM(c.description) FROM c` |
...
| M-07 | `SELECT LOG(c.price) FROM c` |
...
| ID | Query |
|----|-------|
| UDF-01 | `SELECT udf.formatPrice(c.price) FROM c` |
| UDF-02 | `SELECT * FROM c WHERE udf.isExpensive(c.price, 100)` |
| UDF-03 | `SELECT udf.categoryLabel(c.category, c.brand, c.inStock) AS label FROM c` |
...
| N-15 | `SELECT * FROM c WHERE c.price > "hello"` | *(parses OK, type mismatch at runtime)* |
