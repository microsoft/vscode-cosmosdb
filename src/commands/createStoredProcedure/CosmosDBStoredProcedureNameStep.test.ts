...
function validate(name: string | undefined): string | undefined {
    return new CosmosDBStoredProcedureNameStep().validateInput(name);
}

describe('CosmosDBStoredProcedureNameStep.validateInput', () => {
    it('accepts a normal stored procedure name', () => {
        expect(validate('myStoredProc')).toBeUndefined();
    });
    ...
    it.each(['/', '\\', '?', '#', '&'])('rejects the illegal character %j', (char) => {
        expect(validate(`sp${char}name`)).toBe(
            "Stored procedure name cannot contain the characters '\\', '/', '#', '?', '&'",
        );
    });

    it('rejects names longer than 255 characters', () => {
        expect(validate('a'.repeat(256))).toBe('Stored procedure name cannot be longer than 255 characters');
    });
