...
export class DocumentDBPartitionKeyStep extends AzureWizardPromptStep<CreateContainerWizardContext> {
    ...
    public async prompt(context: CreateContainerWizardContext): Promise<void> {
        const placeHolder =
            this.hierarchyStep === HierarchyStep.First
                ? vscode.l10n.t('first partition key e.g., /TenantId')
                : this.hierarchyStep === HierarchyStep.Second
                  ? vscode.l10n.t('second partition key e.g., /UserId')
                  : this.hierarchyStep === HierarchyStep.Third
                    ? vscode.l10n.t('third partition key e.g., /address/zipCode')
                    : 'partition key';
        const prompt = vscode.l10n
            .t(
                'Enter the partition key for the container {0}',
                this.hierarchyStep === HierarchyStep.First ? '' : vscode.l10n.t(`(leave blank to skip)`),
            )
            .trim();
    ...
    public validateInput(partitionKey: string | undefined): string | undefined {
        ...
        if (/[^a-zA-Z0-9_/]/.test(partitionKey)) {
            return vscode.l10n.t(`Partition key cannot contain the wildcard characters`);
        }

        if (!/^\/?[^/]*$/.test(partitionKey)) {
            return vscode.l10n.t('Partition key can only start with a forward slash (/)');
        }

        if (partitionKey.length > 255) {
            return vscode.l10n.t('Partition key cannot be longer than 255 characters');
        }
    ...
    private async validateNameAvailable(
        ...
        if (this.hierarchyStep === HierarchyStep.First && partitionKey.length === 0) {
            return vscode.l10n.t('Partition key is required.');
        }
        ...
        if (context.partitionKey?.paths?.includes(partitionKey)) {
            return vscode.l10n.t('Partition key must be unique.');
        }
