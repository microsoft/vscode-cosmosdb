...
export class MongoAccountTreeItem extends AzureParentTreeItem<IMongoTreeRoot> {
    ...
    constructor(parent: AzureParentTreeItem, id: string, label: string, connectionString: string, isEmulator: boolean, readonly databaseAccount?: CosmosModels.DatabaseAccount) {
        super(parent);
        this.id = id;
        this.label = label;
